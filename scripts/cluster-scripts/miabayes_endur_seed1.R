...
endurmod2.mcmc <- as.data.frame(matrix(NA, nrow=200000, ncol=12))
colnames(endurmod2.mcmc) <- c("Intercept","PreClim","InvClim","InvTime","Phylo","Spp","units","h2phylo","s2","CVphylo","CVspp","CVresid")
endurmod2poly.mcmc <- as.data.frame(matrix(NA, nrow=200000, ncol=9))
colnames(endurmod2poly.mcmc) <- c("Intercept","PreClim","InvClim","InvTime","Spp","units","s2","CVspp","CVresid")
...
while(j<postsize+1){
  ...
  while(cg < length(devs)+1){
    ...
    if(CID<11){ #cristatellus
      slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,3]
    } else { #if the species is a green anole (range 11-20)
      if(CID>10 | CID<21){ #sagrei
        slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,7]
      } else {
        if(CID>20 | CID<31){ #carolinensis
          slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"]
        } else {
          if(CID>30 | CID<43){ #chlorocyanus
            slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,2]
          } else {
            if(CID>42 | CID<53){ #distichus
              slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,5]
            } else {
              if(CID>56| CID<68){ #cybotes
                slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,4]
              } else {
                if(CID>67| CID<78){ #equestris
                  slopeframe[cg,"islps"] <- slopeframe[cg,"devs"] + tmpMod$Sol[i,"nTreatment"] + tmpMod$Sol[i,6]
  ...
  set.seed(330)
  bendmodel2 <- MCMCglmm(islps ~ scPreClim + scInvClim + at.level(dummyInv, "Invasive"):scInvTime,
  ...
  set.seed(330)
  bendmodel2poly <- MCMCglmm(islps ~ scPreClim + scInvClim + at.level(dummyInv, "Invasive"):scInvTime,
                            random=~AbSpec,
                            #ginverse=list(Species=Ainv), 
                            data=iframe,
                            prior=pr2B,
  ...
  ## save posteriors
  tmpbig <- as.data.frame(cbind(bendmodel2$Sol, bendmodel2$VCV))
  tmpbigpoly <- as.data.frame(cbind(bendmodel2poly$Sol,bendmodel2poly$VCV))
  ...
  endurmod2.mcmc[rlow:rhigh,] <- as.data.frame(tmpbig)
  endurmod2poly.mcmc[rlow:rhigh,] <- as.data.frame(tmpbigpoly)  


  #grab relevant output
  tmpsum <- summary(bendmodel2)
  tmpheifx <- heidel.diag(bendmodel2$Sol)
  tmpheiran <- heidel.diag(bendmodel2$VCV)
  tmpsumpoly <- summary(bendmodel2poly)
  tmpheifxpoly <- heidel.diag(bendmodel2poly$Sol)
  tmpheiranpoly <- heidel.diag(bendmodel2poly$VCV)
  tmpacrand <- autocorr.diag(bendmodel2$VCV)[2,]
  tmpacrandpoly <- autocorr.diag(bendmodel2poly$VCV)[2,]
  tmpacfix <- autocorr.diag(bendmodel2$Sol)[2,]
  tmpacfixpoly <- autocorr.diag(bendmodel2poly$Sol)[2,]
  tmphpd <- rbind(HPDinterval(bendmodel2$Sol), HPDinterval(bendmodel2$VCV))
  tmphpdpoly <- rbind(HPDinterval(bendmodel2poly$Sol), HPDinterval(bendmodel2poly$VCV))
  
#random effect sampling correlation for full model
  sampcor[j,1] <- cor(bendmodel2$VCV[,1], bendmodel2$VCV[,2])
...
write.csv(endurmod2.mcmc, file="results/run1/endur_fullmod_mcmc_out.csv")
write.csv(endurmod2poly.mcmc, file="results/run1/endur_reducemod_mcmc_out.csv")

#perform mean calculation while still data frames
for(n in rownames(m2sum)){
  m2sum[n,"post.mean"] <- mean(endurmod2.mcmc[,n])
}

for(n in rownames(polysum)){
  polysum[n,"post.mean"] <- mean(endurmod2poly.mcmc[,n])
}

#convert to mcmc objects to calculate HPD intervals and posterior modes
endurmod2.mcmc <- as.mcmc(endurmod2.mcmc)
endurmod2poly.mcmc <- as.mcmc(endurmod2poly.mcmc)

#calculate HPD and modes
for(n in rownames(m2sum)){
  m2sum[n,"lower.HPD.95"] <- HPDinterval(endurmod2.mcmc[,n])[1]
  m2sum[n,"upper.HPD.95"] <- HPDinterval(endurmod2.mcmc[,n])[2]
  m2sum[n,"post.mode"] <- posterior.mode(endurmod2.mcmc[,n])
}

for(n in rownames(polysum)){
  polysum[n,"lower.HPD.95"] <- HPDinterval(endurmod2poly.mcmc[,n])[1]
  polysum[n,"upper.HPD.95"] <- HPDinterval(endurmod2poly.mcmc[,n])[2]
  polysum[n,"post.mode"] <- posterior.mode(endurmod2poly.mcmc[,n])
}

#calculate pMCMC
for (n in rownames(m2sum)){
  m2sum[n,"pMCMC"] <- 2 * pmax(0.5/dim(endurmod2.mcmc)[1], pmin(colSums(endurmod2.mcmc[,n, drop = FALSE] > 0)/dim(endurmod2.mcmc)[1],
 1 - colSums(endurmod2.mcmc[, n, drop = FALSE] > 0)/dim(endurmod2.mcmc)[1]))
}

for (n in rownames(polysum)){
  polysum[n,"pMCMC"] <- 2 * pmax(0.5/dim(endurmod2poly.mcmc)[1], pmin(colSums(endurmod2poly.mcmc[,n, drop = FALSE] > 0)/dim(endurmod2poly.mcmc)[1],
 1 - colSums(endurmod2poly.mcmc[, n, drop = FALSE] > 0)/dim(endurmod2poly.mcmc)[1]))
}

##--export results
#Model 1 Output
endurmodel1out <- summary(bendmodel1)
sink("results/run1/endur_summ_model1.txt")
print(endurmodel1out)
sink()
endurmode1 <- posterior.mode(bendmodel1$VCV)
write.csv(endurmode1, file="results/run1/endur_mode_1.csv")
endurautocor1 <- autocorr.diag(bendmodel1$VCV)
write.csv(endurautocor1, file="results/run1/endur_autocorr_VCV1.csv")
endurheidelrout1 <- heidel.diag(bendmodel1$VCV)
write.csv(endurheidelrout1, file="results/run1/endur_heidelrout_1.csv")
endurheidelfout1 <- heidel.diag(bendmodel1$Sol)
write.csv(endurheidelfout1, file="results/run1/endur_heidelfout_1.csv")
write.csv(rbind(HPDinterval(bendmodel1$Sol), HPDinterval(bendmodel1$VCV)), file="results/run1/endur_hpds_model1")

#Model 2 Output
write.csv(m2sum, file="results/run1/endur_fullmod_summary.csv")
write.csv(polysum, file="results/run1/endur_reducemod_summary.csv")

write.csv(dicscore, file="results/run1/endur_dics.csv")

write.csv(heidelfout, file="results/run1/endur_heidelfout_longrun.csv")
write.csv(heidelrout, file="results/run1/endur_heidelrout_longrun.csv")
write.csv(autofix, file="results/run1/endur_autocorr_fix_longrun.csv")
write.csv(autorand, file="results/run1/endur_autocorr_rand_longrun.csv")

write.csv(polyheidelfout, file="results/run1/poly_endur_heidelfout_longrun.csv")
write.csv(polyheidelrout, file="results/run1/poly_endur_heidelrout_longrun.csv")
write.csv(polyautofix, file="results/run1/poly_endur_autocorr_fix_longrun.csv")
write.csv(polyautorand, file="results/run1/poly_endur_autocorr_rand_longrun.csv")
